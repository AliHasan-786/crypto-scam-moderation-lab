...
**It runs against the real firehose.** The [live radar](https://crypto-scam-lab.vercel.app/#radar) connects your browser directly to Bluesky's public Jetstream firehose and scores real posts as they are written — client-side, aggregates only, no content stored or displayed, no action possible (Decision Log 011). Offline, the [stream calibration report](audit_outputs/live_stream_calibration.md) runs the full model over a live sample: 8,210 real posts scored from 11,353 observed in a 4-minute window, measuring a 7.1% review-tier share on real traffic against the synthetic assumption — a calibration gap the scale report now states instead of hiding.
...
The public demo is static and sanitized: it cannot publish labels, report accounts, resolve live links, or take any platform action. The live radar reads a public firehose in the visitor's own browser, keeps aggregates only, and has no action path (Decision Log 011). Live Bluesky ingestion beyond that is local, read-only, and opt-in. LLM output is reviewer assistance only, cached for the demo, and gated by faithfulness checks. The dataset is small and mostly English. The eval suites are authored — which is exactly why the error-analysis surface exists. Full limitations in the [case study](case_study/CRYPTO_SCAM_MODERATION_CASE_STUDY.md).
